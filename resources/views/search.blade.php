                                ...
                                <div class="product-container item" itemscope itemtype="https://schema.org/Product">
                                    <div class="left-block">
                                        <h5 class="product-name">
                                            <a href="{{ route('detail',$product->id) }}" title="{{ $product->name }}" itemprop="url" >
                                                {{ $product->name }}
                                            </a>
                                        </h5>
                                        <div class="product-image-container">
                                            <a class= "product_image" href="uploads/products/{{ $product->hinhanh }}" title="{{ $product->name }}">
                                                <img class="img-responsive img1" src="uploads/products/{{ $product->hinhanh }}" alt="{{ $product->name }}" />
                                                {{-- <img  class="img-responsive img2" src="../25-large_default/faded-short-sleeves-tshirt.jpg" alt=""  itemprop="image"  /> --}}
                                            </a>
                                            <a class="new-box" href="{{ route('detail',$product->id) }}">
                                                @if ($product->giamoi)
                                    ...
                                    <div class="right-block">
                                        ...
                                        <div class="media-body">
                                            <div class="content_price pull-left">
                                                 ...
                                                 <span itemprop="price" class="price product-price">{{ $product->giamoi == 0 ? number_format($product->giacu) : number_format($product->giamoi)}}</span>
                                                <meta itemprop="priceCurrency" content="Vnđ" />
                                                @if ($product->giamoi == 0)
                                                
                                                @else
                                                    <span class="old-price product-price">{{number_format($product->giacu)}}</span>
                                                @endif


                                            </div>
                                            <div class="lab-quick-view pull-right">
                                                <a class="quick-view" href="{{ route('detail',$product->id) }}"
                                                   data-id-product="1"
                                                   title="Quick view">
                                        ...
                                        <div class="actions">
                                            <ul class="add-to-links">
                                                {{--<li class="lab-Wishlist pull-left">--}}
                                                    {{--<a onclick="WishlistCart('wishlist_block_list', 'add', {{ $product->id }}, $('#idCombination').val(), 1,'tabproduct'); return false;" class="add-wishlist wishlist_button" href="#"--}}
                                                       {{--data-id-product="{{ $product->id }}"--}}
